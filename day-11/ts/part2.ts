...
const updateGrid = (grid: number[][], endFrame: number, frame = 0, speed = 500) => {
    ...
    const currentFlashTotal = totalFlashes;
    ...
    const nextFlash = totalFlashes - currentFlashTotal;
    console.log('Flashes next frame:', nextFlash);
    
    if(frame < endFrame && nextFlash !== (xSize*ySize))
        setTimeout(() => updateGrid(newGrid, endFrame, ++frame, speed), speed)
}

updateGrid(initGrid, 1000, 0 , 10)